...
function validate(f) {
  ...
  if (!f.permanent_municipality)
    e.permanent_municipality = "Municipality is required";
...
export default function KYCForm() {
  ...
  function handle(field, val) {
    ...
    if (field === "current_province") {
      patch.current_district = "";
      patch.current_municipality = "";
    }
    if (field === "current_district") {
      patch.current_municipality = "";
    }
    ...
    return (
      <div className="page-root flex flex-col items-center justify-center px-6 text-center">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-text-dark mb-2">
            KYC Submitted!
          </h2>
          <p className="text-sm text-text-gray mb-8 leading-relaxed px-2">
            Your application is under review. Verification typically takes 1–3
            business days. We'll notify you once complete.
          </p>
          <div className="flex gap-3">
            <Btn full onClick={() => nav("/kyc/tracking")} variant="primary">
              Track Status
            </Btn>
            <Btn full onClick={() => nav("/chat")} variant="outline">
              Ask AI
            </Btn>
          </div>
        </motion.div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-gray mt-0.5 leading-relaxed">
                  Please review and confirm the details below match your
                  document.
                </p>
              </div>
            </motion.div>
            ...
            {showBanner && ocrResult?.address && (
              <div className="flex items-start gap-2.5 bg-primary/8 border border-primary/15 rounded-xl px-3.5 py-3 mb-4">
                ...
                <p className="text-[11px] text-primary-dark leading-relaxed font-medium">
                  Address information was extracted automatically. Please verify
                  the details below.
                </p>
              </div>
            )}
            ...
            <label className="flex items-center gap-3 cursor-pointer select-none mb-4">
              <div className="relative shrink-0">
                <input
                  ...
                  checked={sameAsPerm}
                  onChange={(e) =>
                    handle("current_same_as_permanent", e.target.checked)
                  }
                  className="sr-only"
                />
                <div
                    ...
                    <p className="text-sm text-text-gray leading-relaxed">
                      {[
                        ...
                        formData.permanent_ward &&
                          `Ward ${formData.permanent_ward}`,
                        formData.permanent_district,
                        formData.permanent_province,
                      ]
                        .filter(Boolean)
                        .join(", ") || "Fill in permanent address above"}
                    </p>
                  ) : (
