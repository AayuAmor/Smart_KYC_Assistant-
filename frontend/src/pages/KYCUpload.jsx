...
import { CloudUpload, CreditCard, BookOpen, Car, Vote, CheckCircle2, ArrowRight } from 'lucide-react'
...
export default function KYCUpload() {
  ...
  const { setDocFile, formData, setFormData } = useKYCStore()
  ...
  function handleFile(file, side) {
    ...
    const updated = { ...previews, [side]: url }
    const allDone = currentDocType.sides.every(s => updated[s])
    if (allDone) setDocFile(file, updated.front)
  }
    ...
    <div className="page-root pb-8">
      ...
      <div className="max-w-[480px] mx-auto px-4 py-5 space-y-4">
        ...
        <div className="flex items-start gap-3 bg-primary/5 border border-primary/15 rounded-2xl px-4 py-3.5">
          ...
          <div>
            <p className="text-sm font-bold text-primary-dark">AI will automatically extract your details</p>
            <p className="text-xs text-primary/70 mt-0.5">No manual typing needed — just upload a clear photo</p>
          </div>
        </div>

        <div>
          <p className="text-xs font-bold text-text-gray uppercase tracking-widest mb-3 px-0.5">Select Document Type</p>
            ...
            {DOC_TYPES.map(({ id, Icon, label, sub }) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Icon size={18} color={active ? '#fff' : '#6B7280'} strokeWidth={2} />
                  </div>
                  <p className="text-xs font-bold text-text-dark leading-tight">{label}</p>
            ...
            return (
              <div key={side}>
                ...
                <div
                  ...
                  onDragOver={e => { e.preventDefault(); setDraggingSide(side) }}
                  ...
                  style={{
                    borderColor: draggingSide === side ? '#60BB46' : preview ? '#60BB46' : '#D1D5DB',
                    background: draggingSide === side ? '#EBF7E6' : preview ? '#F0FDF4' : '#FAFAFA',
                    minHeight: 120,
                  }}
                >
                  {preview ? (
                    <div className="relative">
                      <img src={preview} alt={label} className="w-full object-cover rounded-2xl" style={{ maxHeight: 160 }} />
                      <div
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 py-8 px-4 text-center">
                      ...
                      <p className="text-sm font-semibold text-text-dark">Tap to upload {label}</p>
                      <p className="text-xs text-text-gray">JPG, PNG or PDF — max 10MB</p>
                    </div>
                  )}
                </div>
                <input
        ...
        <div className="bg-amber-50 border border-amber-200/70 rounded-2xl px-4 py-3.5">
          <p className="text-xs font-bold text-amber-800 mb-2">📸 Tips for best results</p>
          <ul className="space-y-1">
            {['Use clear lighting — avoid shadows or glare', 'Ensure all 4 corners of the document are visible', 'Keep the document flat and stable'].map(tip => (
