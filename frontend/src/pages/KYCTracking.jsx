...
import {
  CheckCircle2, Clock, Search, ShieldCheck,
  Upload, MessageSquare, AlertCircle, ArrowRight,
  RefreshCw, X,
} from 'lucide-react'
...
const TIMELINE = [
  ...
  { key: 'approved',   label: 'Approved',             desc: 'KYC fully verified. You are all set!',      Icon: CheckCircle2, color: '#22C55E', bg: '#F0FDF4' },
]
...
const STATUS_MAP = {
  ...
  under_review: { label: 'Under Review', pct: 65,  color: '#F59E0B', badgeBg: '#FFF7ED', badgeText: '#C2410C' },
...
function estimatedTime(createdAt, status) {
  if (!createdAt || status === 'approved' || status === 'rejected') return null
...
export default function KYCTracking() {
  ...
  const fetchStatus = useCallback(async (manual = false) => {
    ...
    try {
      ...
      setRetryCount(c => c + 1)
      if (retryCount >= 2) {
        setError('Unable to fetch status. Check your connection.')
      }
    } finally {
      if (manual) setRefreshing(false)
    }
  }, [kycId, retryCount, setKycStatus, nav])

  useEffect(() => {
    if (!kycId) return
    fetchStatus()
    if (kycStatus === 'approved' || kycStatus === 'rejected') return
    const poll = setInterval(() => fetchStatus(), 5000)
    return () => clearInterval(poll)
  }, [kycId, kycStatus])

  const stageIdx = STATUS_STAGE[kycStatus] ?? 0
  const st = STATUS_MAP[kycStatus] || STATUS_MAP.pending
  const eta = estimatedTime(createdAt, kycStatus)
        ...
        right={
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={refreshing}
            >
              <RefreshCw size={14} className={refreshing ? 'animate-spin text-primary' : 'text-slate-500'} strokeWidth={2.5} />
            </button>
            <span
        ...
        <AnimatePresence>
          {error && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -8 }}
              className="bg-red-50 border border-red-200 rounded-2xl px-4 py-3 flex items-center justify-between"
            >
        ...
        {kycId && (
          <div className="bg-white border border-slate-100 rounded-2xl px-4 py-3 flex items-center justify-between shadow-sm">
            ...
            <div className="text-right">
              ...
              {createdAt && (
                <p className="text-[10px] text-text-gray mt-0.5">Submitted {formatDate(createdAt)}</p>
              )}
            </div>
          </div>
        )}
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1">
                <h3 className="text-base font-black text-text-dark mb-1">
                  {kycStatus === 'approved' ? 'KYC Approved!' : kycStatus === 'under_review' ? 'Under Review' : 'Submitted'}
                </h3>
                  ...
                  <div className="flex flex-col items-center shrink-0">
                    ...
                    >
                      {isDone ? (
                        ...
                        <Clock size={16} color="#F59E0B" strokeWidth={2.2} className="animate-pulse" />
                      ) : (
                    ...
                    {i < TIMELINE.length - 1 && (
                      <div
                        ...
                        style={{
                          height: 28,
                          background: isDone ? `linear-gradient(to bottom, ${item.color}, ${item.color}60)` : '#E5E7EB',
                        }}
                      />
                    )}
                  </div>
                  ...
                  <div className={['flex-1', i < TIMELINE.length - 1 ? 'pb-1' : ''].join(' ')}>
                    <div className="pt-2 pb-5">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isDone && lastUpdated && i === stageIdx && (
                          <span className="text-[10px] font-semibold text-text-gray bg-slate-50 px-1.5 py-0.5 rounded-full">
                            {timeAgo(lastUpdated)}
                          </span>
                        )}
                      </div>
        ...
        {eta && (
          <div className="flex items-center gap-3 bg-primary/5 border border-primary/15 rounded-2xl px-4 py-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-primary/70 mt-0.5">Based on your submission time</p>
            </div>
            <span className="ml-auto text-sm font-black text-primary shrink-0">{eta}</span>
          </div>
        )}
